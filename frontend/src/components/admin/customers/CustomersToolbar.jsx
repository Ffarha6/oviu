...
export default function CustomersToolbar({ search, onSearchChange, status, onStatusChange, role, onRoleChange }) {
  return (
    <div className="flex flex-wrap items-center gap-3">
      <div className="flex-1 min-w-[220px] flex items-center gap-2 bg-background border border-primary/10 rounded-xl px-3.5 py-2.5">
        ...
        <input
          ...
          placeholder="ابحثي بالاسم أو البريد أو الهاتف..."
          className="bg-transparent outline-none text-sm flex-1 text-primary placeholder:text-primary/40"
        />
      </div>

      <SelectDropdown value={status} onChange={onStatusChange} options={statusOptions} />
      <SelectDropdown value={role} onChange={onRoleChange} options={roleOptions} />
    </div>
  );
}
